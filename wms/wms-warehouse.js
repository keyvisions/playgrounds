...
class WMSWarehouse extends HTMLElement {
	constructor() {
		super();
		...
		this._onResize = () => this._alignSVG();
	}

	static get observedAttributes() {
		return ["planimetry", "inventory", "locations", "data", "mode"];
	}

	connectedCallback() {
		const validModes = ["picking", "putaway", "edit"];
		const queryMode = this._norm(this._queryParams.get("mode"));
		const attrMode = this._norm(this.getAttribute("mode"));
	...
	attributeChangedCallback(name, oldValue, newValue) {
		...
		if (name === "mode") {
			this._syncSizeControlVisibility();
		}
	...
	_render() {
		...
		const querySize = this._getRequestedSizeFromQuery() ?? 1;
		const sizeRadios = this.shadowRoot.querySelectorAll('input[name="size"]');
		for (const radio of sizeRadios) {
			radio.checked = radio.value === String(querySize);
			radio.addEventListener("change", () => this._onSizeChange());
		}
		this._syncSizeControlVisibility();

		const obj = this.shadowRoot.querySelector("object");
		const planimetry = this.getAttribute("planimetry");
		if (planimetry) obj.data = planimetry;
	...
	_onSizeChange() {
		const requestedSize = this._getRequestedSize();
	...
	get _svg() {
		return this._svgDoc?.querySelector("svg") ?? null;
	}

	// The _setup method transforms a raw SVG into a structured format with <rect> elements for racks and a clean viewBox.
	...
	_emitSvgChange() {
		const svg = this._svg;
		if (!svg) return;
		const planimetry = svg.outerHTML.replace(` xmlns=""`, "").replaceAll(/\s+/g, " ").trim();
		this.dispatchEvent(new CustomEvent("svgchange", { detail: planimetry, bubbles: true }));
		if (this.getAttribute("mode") === "edit")
			this._copyToClipboard(planimetry);
	}
	...
	_attachSvgEvents(svg) {
		...
		svg.addEventListener("click", event => {
			...
			if (target?.tagName === "svg") {
				const hit = this._svgDoc?.elementFromPoint?.(event.clientX, event.clientY);
				target = hit?.closest?.("rect.rack") ?? target;
			}
			if (target?.tagName === "rect") {
				if (this.getAttribute("mode") === "edit")
					this._editRack(target, event.ctrlKey);
				else
					this._showStorageUnit({ target });
	...
	async _localizeRack(inventory) {
		...
		if (this.getAttribute("mode") === "putaway") {
			const requestParams = this._requestApiParams();
			const inventoryData = Array.isArray(inventory)
				? inventory
				: await this._loadDataAttribute("inventory", [], requestParams);
			const locationsData = await this._loadDataAttribute("locations", {}, requestParams);
			this._colorizeRacks(
				this._asArray(inventoryData),
		...
		if (!Array.isArray(inventory)) {
			const requestParams = this._requestApiParams();
			const loadedInventory = await this._loadDataAttribute("inventory", [], requestParams);
			inventory = this._asArray(loadedInventory);
		}
		if (this.getAttribute("mode") === "picking") {
			const requestParams = this._requestApiParams();
			const pickingData = await this._loadDataAttribute("data", null, requestParams);

			if (pickingData != null) {
				const pickRows = this._normalizePickingDataRows(pickingData);
				const matchedLocations = new Set();

				for (const row of pickRows) {
					if (typeof row !== "object" || row === null) continue;

					this._splitCsvValues(row.location).forEach(loc => matchedLocations.add(loc));

					const partnumbers = this._splitCsvValues(row.partnumber).map(v => v.toLowerCase());
					const lus = this._splitCsvValues(row.lu).map(v => v.toLowerCase());
					if (!partnumbers.length && !lus.length) continue;

					this._matchInventoryLocations(inventory, partnumbers, lus, matchedLocations);
				}

				inventory = [...matchedLocations];
			} else {
				const qs = this._queryParams;
				const qsLocations = this._splitCsvValues(qs.get("location"));
				const qsPartnumbers = this._splitCsvValues(qs.get("partnumber")).map(v => v.toLowerCase());
				const qsLus = this._splitCsvValues(qs.get("lu")).map(v => v.toLowerCase());

				if (qsPartnumbers.length || qsLus.length) {
					inventory = [...this._matchInventoryLocations(inventory, qsPartnumbers, qsLus, new Set(qsLocations))];
				} else {
		...
		inventory.forEach(location => {
			...
			if (rack)
				...
				// alert(`Ubicazione ${this._formatLocation(location)} non trovata.`);
				svgDoc.firstElementChild.style.border = "thick solid red"
		});
	}

	_normalizePickingDataRows(data) {
	...
	_splitCsvValues(value) {
		if (Array.isArray(value))
			return value
				.flatMap(v => this._splitCsvValues(v))
				.filter(Boolean);
	...
	_matchInventoryLocations(inventory, partnumbers, lus, seed = new Set()) {
		for (const item of this._asArray(inventory)) {
			if (typeof item !== "object" || item === null) continue;
			const loc = String(item.location ?? "").trim();
			if (!loc) continue;
			if ((partnumbers.length ? partnumbers.includes(this._norm(item.partnumber)) : true) &&
				(lus.length ? lus.includes(this._norm(item.lu)) : true))
				seed.add(loc);
		}
		return seed;
	}

	_requestApiParams(extra = {}) {
	...
	_buildApiUrl(value, extraParams = {}) {
		const url = new URL(value, window.location.href);
		const params = this._requestApiParams(extraParams);
	...
	async _loadDataAttribute(name, fallback, extraParams = {}) {
		...
		const url = this._buildApiUrl(value, extraParams);
		const key = url.href;
	 ...
	 */
	_colorizeRacks(inventory, locations) {
		const svgDoc = this._svgDoc;
		if (!svgDoc) return;

		const partnumber = this._getQueryPartnumber();
		const requestedSize = this._getRequestedSize();
		...
		const hitSet = new Set(
			(partnumber
				? inventory.filter(item =>
					...
					this._norm(item.partnumber) === partnumber &&
		...
		svgDoc.querySelectorAll("rect.rack").forEach(rack => {
			...
			const codes = this._rackLocations(rack);
			...
			if (requestedSize !== null) {
				...
				for (const c of codes) {
					...
					if (!this._isValidOccupancy(v)) continue;
					hasKnown = true;
	...
	_getQueryPartnumber() {
		return this._norm(this._queryParams.get("partnumber"));
	}

	_getRequestedSizeFromQuery() {
		const raw = this._queryParams.get("size");
	...
	_getRequestedSize() {
		...
		return this._getRequestedSizeFromQuery();
	}
	...
	_editRack(target, autoAssign = false) {
		...
		this._svg.querySelectorAll("title").forEach(t => {
	...
	async _showStorageUnit(event) {
		const target = event.target;
		const storageUnit = StorageUnits.find(unit => unit.alias === target.id[0]);
		if (!storageUnit) return;

		if (storageUnit.layout === "horizontal") {
			for (const rack of this._svgDoc.querySelectorAll(`rect[id^='${target.id[0]}']`)) {
			...
			try {
				const inventory = await this._loadDataAttribute("inventory", [], extraParams);
				let locationStates = {};
				if (mode === "putaway") {
					const loaded = await this._loadDataAttribute("locations", {}, extraParams);
					if (loaded && typeof loaded === "object" && !Array.isArray(loaded))
						locationStates = loaded;
				}
				this._openStorageUnitDialog(target, storageUnit,
					this._asArray(inventory),
					{ mode, locationStates });
			} catch (err) {
				console.error("Error fetching storage unit data", err);
				this._openStorageUnitDialog(target, storageUnit, [], { mode });
			}
		}
	}

	_binContent(item, standard = false) {
	...
	_openStorageUnitDialog(target, storageUnit, inventory, context = {}) {
			...
			.map(j => {
				const cells = bays.map(b => {
					...
					const hasPartnumber = partnumber
						? items.some(item => this._norm(item?.partnumber) === partnumber)
						: false;
					const rawOccupancy = locationStates?.[locCode];
					const occupancy = rawOccupancy == null ? Number.NaN : Number(rawOccupancy);
					const validOccupancy = this._isValidOccupancy(occupancy);
		...
		if (mode === "putaway") {
			dialog.addEventListener("click", e => {
				...
				const occupancy = prompt(`Quanto è piena l'ubicazione ${this._formatLocation(location)}? [0-5] N.B. Se non è utlizzata lasciare vuoto.`, td.dataset.occupancy);
				if (occupancy === null) return;
				const n = Number.parseInt(occupancy, 10);
				if (Number.isNaN(n) || n < 0 || n > 5) return;
				locationStates[location] = n;
				this._colorizeRacks(inventory, locationStates);
			});
		}
	}
}
