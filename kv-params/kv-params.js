...
class kvParams extends HTMLElement {
  static observedAttributes = [
    ...
    "columns",
    "template",
  ]; // mode = [ *show | edit | range | manage | template] um = [ *msu | isu ]
  static #Texts = {
    ...
    "delete": {
      en: "You sure you want to delete the parameter?",
      it: "Sicuri di voler eliminare il parametro?",
    },
    "optionless": {
      en: "Options are not applicabile to $1 parameters.",
      it: "Le opzioni non sono applicabili ai parametri $1.",
    },
    "tests": { en: "Test", it: "Test" },
    "testRemove": {
      en: "Sure you want to delete the test?",
      it: "Sicuri di voler eliminare il test?",
    },
    "na": {
      en: "Allow Not Applicable (NA)",
      it: "Consenti Non Applicabile (NA)",
    },
    ...
    "showhide": {
      en: "Show only required parameters",
      it: "Mostra solo parametri obbligatori",
    },
    "move": {
      en:
        "To move a parameter up/down, select it, then press Alt+Up arrow or Alt+Down arrow",
      it:
        "Per spostare un parametro su/giù, selezionalo, quindi premi Alt+Freccia Sù o Alt+Freccia Giù",
    },
  };
  static #Flags = [
  ...
  constructor() {
    ...
    this.addEventListener("mouseup", (event) => {
      ...
      if (
        target.name === "showhide" ||
        target.firstElementChild?.name === "showhide"
      ) {
        if (target.name !== "showhide") target = target.firstElementChild;
        target.closest("kv-params").querySelector("table").querySelectorAll(
      ...
      target.closest("kv-params tbody")?.querySelector(".selected")?.classList
        .remove("selected");
      ...
      if (target.classList.contains("tests")) {
        ...
        this.querySelector("thead th[colspan]").setAttribute(
          "colspan",
          colspan - 1,
        );
        ...
        this.querySelectorAll("tbody>tr").forEach((row) => {
          if (row.children[0].hasAttribute("colspan")) {
            ...
            node.children[0].setAttribute(
              "tabindex",
              count + parseInt(node.children[0].getAttribute("tabindex")),
            );
        ...
        if (colspan > 2) {
          ...
          this.querySelector("tfoot tr").lastElementChild.insertAdjacentHTML(
            "afterend",
            '<th><i class="far fa-trash-alt testRemove"></i></th>',
          );
        }
        ...
        if (colspan == 2) {
          ...
          this.querySelector("tfoot tr").lastElementChild.previousElementSibling
            .remove();
        }
        ...
        target.closest("tbody")?.children.length != 1 &&
        (event.ctrlKey || confirm(kvParams.#Texts.delete[this.Lang]))
      ) {
        ...
        target.closest("td").children[0].value =
          target.classList.contains("deselected")
            ? flags & ~(1 << target.getAttribute("value"))
            : flags | (1 << target.getAttribute("value"));
      } else if (target.classList.contains("range")) {
        const range = [
          isNaN(parseFloat(target.form.min.value))
            ? null
            : parseFloat(target.form.min.value),
          isNaN(parseFloat(target.form.max.value))
            ? null
            : parseFloat(target.form.max.value),
          isNaN(parseInt(target.form.decimals.value))
            ? null
            : parseInt(target.form.decimals.value),
          Array.from(target.form.querySelectorAll('[name="flag"]')).reduce(
            (acc, flag, i) => {
        ...
        if (target.options.hasAttribute("range")) {
          ...
          target.options.setAttribute(
            "step",
            range[2] == null ? "any" : Math.pow(10, -range[2]),
          );
        } else {
        ...
        const options = range ||
          kvParams.findId(
            this.Schema,
            target.getAttribute("ref").replace("P", ""),
          ).options;
        const span = this.querySelector(
          `[title="${target.getAttribute("ref").replace("P", "@")}"] span`,
        );
        if (span) {
        ...
        if (!target.classList.contains("set")) {
          ...
          this.querySelectorAll("tbody>tr").forEach((el) =>
            el.style.display = ""
          );
        }

        // Set kvParamsSheet cookie
        document.cookie = `kvParamsSheet=${
          parseInt(target.getAttribute("data-flag")) || 4
        }; path=/; max-age=31536000; SameSite=Strict`; // Cookie valid for 1 year
      }
    });
    ...
    document.addEventListener("keyup", (event) => {
      if (
        event.key == "Enter" && event.target.classList.contains("manageOptions")
      ) {
        this.#manageOptions(event);
      } else if (
        (event.key == "Enter" || event.key == " ") &&
        event.target.tagName === "I" && event.target.hasAttribute("tabindex")
      ) {
        event.target.dispatchEvent(
          new MouseEvent("mouseup", { bubbles: true }),
        );
      } else if (
        this.Mode == "manage" && event.altKey &&
        ...
        if (event.key == "ArrowUp" && row.previousElementSibling) {
          ...
          row.parentNode.insertBefore(
            nextRow || row.parentNode.children[0],
            row,
          );
        }
  ...
  async #fetchData(name, json) {
    if (name == "schema" && this.Schema.length == 0) {
      await fetch(json).then((res) => res.json()).then((data) =>
        this.Schema = data || []
      ).catch(() => this.Schema = []);
    } else if (name == "data") {
      await fetch(json).then((res) => res.json()).then((data) =>
  ...
  async attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      ...
      case "um":
        ...
        break;
      case "schema":
        await this.#fetchData(name, newValue);
        break;
      case "data":
        await this.#fetchData(name, newValue);
        break;
    ...
    this.innerHTML = "";
    if (newValue == "manage") {
      const sheetCookie = document.cookie.split("; ").find((row) =>
        row.startsWith("kvParamsSheet=")
      );
      if (sheetCookie) {
  ...
  #manageSchema() {
    const html = `<input type="hidden" name="${this.getAttribute("name")}">` +
      `<div style="font-size:smaller"><i class="fas fa-fw fa-info-circle"></i> ${
        kvParams.#Texts.move[this.Lang]
      }</div>` +
      `<table class="schema">` +
      `<thead>` +
      `<tr>` +
      ...
      `<th style="white-space:nowrap">${
        kvParams.#Flags.map((el, i) =>
          `<i class="${el.icon}" title="${el[this.Lang]}" data-flag="${i}"></i>`
        ).join(" ")
      }</th>` +
      `<th style="white-space:nowrap"><i class="far fa-trash-alt" style="visibility:hidden"></i> <i class="fas fa-plus add" tabindex="0"></i></th>` +
      `</tr>` +
      "</thead>" +
      ...
      `<option value="checkbox">${
        kvParams.#Texts.checkbox[this.Lang]
      }</option>` +
      `<option value="textarea">${
        kvParams.#Texts.textarea[this.Lang]
      }</option>` +
      `<option value="group">${kvParams.#Texts.group[this.Lang]}</option>` +
      `<option value="separator">${
        kvParams.#Texts.separator[this.Lang]
      }</option>` +
      `</select>` +
      `</td>` +
      `<td><input form="kv-params" name="label" title="Label" data-value="${
        JSON.stringify({
          "en": kvParams.#Texts.new[0],
          "it": kvParams.#Texts.new[1],
        })
      }" style="width: -webkit-fill-available" value="${
        kvParams.#Texts.new[this.Lang]
      }"></td>` +
      `<td>` +
      `<select form="kv-params" name="um" title="UM">` +
      `<option></option>` +
      UMS.map((um) =>
        `<option value="${um.id}">${um.msu} ${
          um.isu?.replace(/(.*)/, " [$1]") || ""
        }</option>`
      ).join("") +
      `</select>` +
      ...
      kvParams.#Flags.map((flag, i) =>
        `<i class="${flag.icon} deselected" name="flags" value="${i}" title="${
          flag[this.Lang]
        }" tabindex="0"></i>`
      ).join(" ") +
      `</td>` +
      ...
      row.querySelectorAll("input,select").forEach((el) => {
        if (el.name == "label") {
          ...
          for (let i = 0; i < kvParams.#Flags.length; ++i) {
            if ((param.flags & (1 << i))) {
              row.querySelector(`i[name="flags"][value="${i}"]`).classList
                .remove("deselected");
            } else {
              row.querySelector(`i[name="flags"][value="${i}"]`).classList.add(
                "deselected",
              );
            }
          }
          ...
          el.checked = (param[el.name] & (1 << parseInt(el.value))) == 0
            ? false
            : true;
        } else {
  ...
  #newRow(sibling) {
    ...
    if (sibling.firstChild.tagName == "TH") {
      flag = parseInt(sibling.querySelector(".set").dataset.flag);
    ...
    row.querySelector('input[name="label"]').setAttribute(
      "data-value",
      JSON.stringify(kvParams.#Texts.new),
    );
    row.querySelector('input[name="label"]').value =
      kvParams.#Texts.new[this.Lang];
    row.querySelector('select[name="um"]').value = "";
    row.querySelector('input[name="options"]').value = "";
    row.querySelector(".manageOptions").classList.remove("set");
    if (flag) {
      row.querySelector(`[style="white-space:nowrap"]`).innerHTML = kvParams
        .#Flags.map((el, i) =>
          `<i class="${el.icon}${flag === i ? '' : ' deselected'}" name="flags" value="${i}" title="${
            el[this.Lang]
          }" tabindex="0"></i>`
        ).join(" ");
    }
  ...
  #manageOptions(event) {
    ...
    const um = kvParams.findId(
      UMS,
      row.querySelector("[name=um]")?.value || event.target.getAttribute("um"),
    );

    row.closest("tbody").querySelector(".selected")?.classList.remove(
      "selected",
    );
    ...
    switch (row.querySelector("[name=type]")?.value || "number") {
      case "number":
        if (event.target.getAttribute("ref") == "options") {
          options = JSON.parse(
            row.querySelector('[name="options"]')?.value ||
              "[null, null, null, null]",
          );
        } else {
          param = kvParams.findId(
            this.Schema,
            event.target.getAttribute("ref").substring(1),
          );
          try {
            options = JSON.parse(
              row.querySelector(`[name=${event.target.getAttribute("ref")}]`)
                ?.getAttribute("range"),
            ) || param?.options;
          } catch {
        ...
        options[0] = isNaN(parseFloat(options[0]))
          ? ""
          : parseFloat(options[0]);
        options[1] = isNaN(parseFloat(options[1]))
          ? ""
        ...
        html += `</header><form>` +
          `<label><span>Min</span> <input type="number" name="min" step="any" value="${
            options[0]
          }"></label>` +
          `<label><span>Max</span> <input type="number" name="max" step="any" value="${
            options[1]
          }"></label>` +
          `<label><span>${
            kvParams.#Texts.decimals[this.Lang]
          }</span> <input type="number" name="decimals" min="0" max="5" step="1" value="${
            options[2]
          }"></label>` +
          `<p style="margin:0.5rem 0"><label><input type="checkbox" name="flag" ${
            (options[3] & 1) ? "checked" : ""
          }> ${
            kvParams.#Texts.na[this.Lang]
          }</label><br><label><input type="checkbox" name="flag" ${
            (options[3] & 2) ? "checked" : ""
          }> ${kvParams.#Texts.required[this.Lang]}</label></p>` +
          `<button type="button" class="range" ref="${
            event.target.getAttribute("ref")
          }" id="kv-params-options" style="float:right">${
            kvParams.#Texts.save[this.Lang]
          }</button>` +
          `</form></dialog>`;
        break;
      ...
      case "select":
        ...
        html =
          ...
          `<kv-pair name="options" src="${
            row.querySelector("[name=options]").value.replaceAll('"', "&quot;")
          }" labels="${um ? um.msu + "," + um.isu : "en,it"}"></kv-pair>` +
          `<input type="checkbox" ${
            options[1] ? "checked" : ""
          } onchange="this.previousElementSibling.setAttribute('pair', this.checked)"><label> ${
            kvParams.#Texts.pair[this.Lang]
          }</label><br>` +
          `<button type="button" ref="${
            event.target.getAttribute("ref")
          }" class="options" id="kv-params-options" style="float:right">${
            kvParams.#Texts.save[this.Lang]
          }</button>` +
          `</form></dialog>`;
        break;
      ...
      default:
        alert(
          kvParams.#Texts.optionless[this.Lang].replace(
            "$1",
            row.querySelector("[name=type]").value,
          ),
        );
        return;
    }
    ...
    if (html) {
      ...
      document.getElementById("kv-params-options").options =
        row.querySelector("[name=options]") ||
  ...
  #manageSheet(mode, templateId) {
    ...
    this.Schema.forEach((param) => {
      if (
        param.flags & (1 << sheet) &&
        (mode != "range" || (mode == "range" && param.type == "number"))
      ) {
    ...
    this.Schema.forEach((param, j) => {
      if (
        ...
        const um = param.um
          ? "[" +
            (ums[(mode == "edit" || mode == "range") ? "msu" : this.UM] ||
              ums.msu) +
            "]"
          : "";
        const icons = (param.flags & 1 ? '<i class="fas fa-font"></i>' : "") +
          (param.flags & 1 ? '<i class="fas fa-search"></i>' : "");
        ...
        switch (param.type) {
          case "number":
            options = this.Data[`PR${param.id}`] || [param.options] ||
              [[null, null, null, null]];
            html += `<tr><td title="@${param.id}">${
              param.label[this.Lang]
            } ${um} ${icons}<span>${this.#range(options[0])}</span></td>`;
            kvParams.sizeArray(this.Data[`P${param.id}`], cols).forEach(
              (value, k) => {
                value = (mode == "show" && this.UM != "msu" &&
                    typeof ums?.convert == "function"
                  ? ums.convert(value).toFixed(options[3])
                  : value) || "";
                html += `<td><input type="${
                  mode == "range" ? "hidden" : "number"
                }" ${attributes(param)} value="${value}" range="${
                  JSON.stringify(options[k] || []).replaceAll('"', "&quot;")
                }" tabindex="${k * count + j}">`;
                if (mode == "range") {
                  html +=
                    `<i class="fas fa-sliders-h manageOptions" ref="P${param.id}" um="${param.um}" tabindex="${
                      k * count + j
                    }"></i>`;
                }
                html += "</td>";
          ...
          case "text":
            ...
            kvParams.sizeArray(this.Data[`P${param.id}`], cols).forEach(
              (value, k) => {
                html += `<td><input ${attributes(param)} value="${
                  value || ""
                }" tabindex="${k * count + j}"></td>`;
              },
            );
            html += "</tr>";
            break;
          ...
          case "date":
            ...
            kvParams.sizeArray(this.Data[`P${param.id}`], cols).forEach(
              (value, k) => {
                html += `<td><input type="date" ${attributes(param)} value="${
                  value || ""
                }" tabindex="${k * count + j}"></td>`;
              },
            );
          ...
          case "checkbox":
            ...
            kvParams.sizeArray(this.Data[`P${param.id}`], cols).forEach(
              (value, k) => {
                html += `<td><input type="checkbox" ${attributes(param)} ${
                  value ? "checked" : ""
                } tabindex="${k * count + j}"></td>`;
              },
            );
          ...
          case "select": {
            ...
            switch (mode) {
              ...
              default:
                i = param.options[1] &&
                    (param.um && this.UM == "isu" ||
                      !param.um && this.Lang != "en")
                  ? 1
                  : 0;
            }
            html += `<tr><td title="@${param.id}">${
            ...
            kvParams.sizeArray(this.Data[`P${param.id}`], cols).forEach(
              (value, k) => {
                ...
                html += `<td><select ${attributes(param)} tabindex="${
                  k * count + j
                }"><option></option>${
                  param.options[i]?.split(",").map((option, j) =>
                    `<option value="${ref[j].replace(/"/g, "&quot;")}" ${
                      l == j ? " selected" : ""
                    }>${option}${
                      paired[j] ? " [" + paired[j] + ums.isu + "]" : ""
                    }</option>`
                  ).join("")
                }</select></td>`;
              },
          ...
          case "textarea": {
            const value = typeof this.Data[`P${param.id}`] != "undefined"
              ? this.Data[`P${param.id}`][0]
              : "";
            html += `<tr><td colspan="*" title="@${param.id}"><label><span>${
              param.label[this.Lang]
            }${param.um ? ` [${param.um}]` : ""} ${icons}</span><textarea ${
              attributes(param)
            } tabindex="${j}">${value}</textarea></label></td></tr>`;
            break;
          }
          case "separator":
          ...
          case "group":
            html += `<tr class="colspan"><td colspan="*">${
              param.label[this.Lang]
            }</td></tr>`;
            break;
        }
    ...
    html += `</tbody><tfoot><tr>${"<th></th>".repeat(cols)}<th>${
      (sheet == 7 && mode == "edit" && cols > 1)
        ? '<i class="far fa-trash-alt testRemove"></i>'
        : ""
    }</th></tr></tfoot>`;
    this.insertAdjacentHTML(
      "afterbegin",
      `<div style="columns:${this.Columns};"><table class="test"><thead ${
        mode == "manage" || sheet == 7 ? "" : 'style="display:none"'
      }><tr><th style="width:100%;text-align:left" title="${
        kvParams.#Flags[sheet][this.Lang]
      }"></th><th${
        (sheet == 7 && (mode == "edit" || mode == "range"))
          ? ` class="tests" colspan="${cols}">${
            kvParams.#Texts.tests[this.Lang]
          } <i class="fas fa-plus tests"></i>`
          : ` colspan="${cols}">`
      }</th></tr></thead><tbody class="kv-params">` +
    ...
    this.insertAdjacentHTML(
      "afterbegin",
      `<label><input form="none" name="showhide" type="checkbox"> ${
        kvParams.#Texts.showhide[this.Lang]
      }</label>`,
    );

    const target = this.querySelector('[name="showhide"]');
    target.closest("kv-params").querySelectorAll(
      "table :is(input, select, textarea):not([required])",
    ).forEach((el) =>
      el.closest("tr").style.display = target.checked ? "none" : ""
    );
    ...
    function attributes(param) {
      ...
      if (param.type == "number") {
        range = `${
          isNaN(parseFloat(options[0])) ? "" : ` min="${options[0]}"`
        }${isNaN(parseFloat(options[1])) ? "" : ` max="${options[1]}"`}${
          isNaN(parseInt(options[2]))
            ? ' step="any"'
            : ` step="${Math.pow(10, -parseInt(options[2]))}"`
        }`;
      }
      return `name="P${param.id}"${
        (param.flags & (1 << 10)) == 0 ? "" : " required"
      }${
        (param.flags & (1 << 3)) == 0 ? "" : " disabled"
      } ${range} form="kvParams"`;
    }
  }
  #manageTemplate(mode, template) {
    ...
    this.querySelectorAll('slot[name^="P"]').forEach((el) => {
      ...
      const param = kvParams.findId(
        this.Schema,
        parseInt(paramName.substring(1)),
      );

      if (param) {
        const ums = kvParams.findId(UMS, param.um);
        const um = param.um
          ? "[" + (ums[mode == "edit" ? "msu" : this.UM] || ums.msu) + "]"
          : "";
        const icons = (param.flags & 1 ? '<i class="fas fa-font"></i>' : "") +
          (param.flags & 1 ? '<i class="fas fa-search"></i>' : "");
        ...
        switch (param.type) {
          case "number":
            ...
            el.insertAdjacentHTML(
              "afterend",
              `<label title="@${param.id}"><span>${
                param.label[this.Lang]
              } ${um} ${icons} ${this.#range(options[0])}</span><input ${
                attributes(param)
              } value="${value}" range="${
                JSON.stringify(options[k] || []).replaceAll('"', "&quot;")
              }"></label>`,
          ...
          case "text":
            el.insertAdjacentHTML(
              ...
              `<label title="@${param.id}"><span>${
                param.label[this.Lang]
              } ${um} ${icons}</span> <input ${attributes(param)} value="${
                value || ""
              }"></label>`,
            );
            break;
          ...
          case "checkbox":
            el.insertAdjacentHTML(
              ...
              `<label title="@${param.id}"><span>${
                param.label[this.Lang]
              } ${um} ${icons}</span> <input type="checkbox" ${
                attributes(param)
              } ${value ? "checked" : ""}></label>`,
            );
            break;
  ...
  #range(options) {
    return `${
      isNaN(parseFloat(options[0]))
        ? "(-∞,"
        : `[${parseFloat(options[0]).toFixed(parseInt(options[2]))}, `
    } ${
      isNaN(parseFloat(options[1]))
        ? "+∞)"
        : `${parseFloat(options[1]).toFixed(parseInt(options[2]))}]`
    }${
      isNaN(parseInt(options[2]))
        ? ""
        : ` ±${Math.pow(10, -parseInt(options[2]))}`
    }`;
  }
  save() {
    if (this.querySelector(".schema") != null) {
      ...
      let max = this.Schema.reduce((maxId, el) =>
        Math.max(maxId, parseInt(el.id)), 0) || 0;
      ...
      this.querySelectorAll("input:not([name=showhide]), select, textarea")
        .forEach((el, i) => {
          const s = i % tests;
          if (!s) {
          ...
          if (el.type == "checkbox") {
            ...
            this.Data[el.name.replace("P", "PR")][s] = kvParams.sizeArray(
              JSON.parse(el.getAttribute("range")),
              4,
            );
          } else if (el.hasAttribute("data-value")) {
            this.Data[el.name][s] = JSON.parse(
              el.dataset.value || '{"en":null,"it":null}',
            );
      ...
      if (
        this.closest("form") && this.getAttribute("name") &&
        this.closest("form")[this.getAttribute("name")]
      ) {
        this.closest("form")[this.getAttribute("name")].value = JSON.stringify(
          this.Data,
        );
      }
